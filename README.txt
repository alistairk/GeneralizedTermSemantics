...
data/sampleTagged.txt	-- sample of Stanford POS tagged data
...
A similar program builds matrices using POS tagged text. This version uses language specific code for English, French and German. 
This program the minimum count of each word and context is its the number of other contexts/words it appears with, not its total
count. A window size must also be specified. In my work I used 5. Tagging must be done with the Stanford POS tagger.
...
example command:
$ java -cp GenTS.jar:. ca.akennedy.gents.buildmatrix.BuildMatrixPOSTagged en N . newMatrix2 20 20 5 sampleTagged.txt
...
These programs require divisi2

python/lsa.py <singular values to generate> <row matrix>
python/formatLSAMatrix.py <lsa file name> <singular values to use> <exponent to attach to sigma>
