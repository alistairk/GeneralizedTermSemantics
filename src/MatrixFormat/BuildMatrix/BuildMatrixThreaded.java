...
/**
 * This class is a threaded version of BuildMatrix. It works more or less
 * the same, except you can specify how many threads you want to run at a 
 * time. Threads are for reading the parsed data in parallel and counting
 * the contexts and words.
 * 
 * @author akennedy
 *
 */
public class BuildMatrixThreaded implements Runnable {
	...
	private static int min_columns;
	private static int min_rows;
	
	private static int wordCounter;
	private static int contextCounter;
	private static int nonZeroEnties;
	
	private static TreeMap<String, Integer> word2Index;
	private static TreeMap<String, Integer> context2Index;
	
	private static ArrayList<String> index2Word;
	private static ArrayList<String> index2Context;
	
	private static ArrayList<Integer> wordCount; //index is word ID
	private static ArrayList<Integer> contextCount; //index is context ID
	private static ArrayList<TreeMap<Integer, Integer>> pairCounter; // ArrayList index is Word ID, TreeMap index is context ID
	
	private static int[] rowSorted2original;
	private static int[] columnSorted2original;
	

	private static int[] rowOriginal2sorted;
	private static int[] columnOriginal2sorted;
	
	private static int rowCount;
	private static int columnCount;
	private static int entryCount;
	
	private static String previousFeatureType;
	
	
	/**
	 * Takes arguments as follows:
	 * <N|V|A> <output Directory> <output File Name> <min Term Frequency> <min Context Frequency> <num Threads> <parsedFile 1> ... <parsedFile n>
	 * 
	...
	public static void main(String[] args) {
		if(args.length < 7){
			System.out.println("To Run Program: java BuildMatrixThreaded <N|V|A> <output Directory> <output File Name> <min Term Frequency> <min Context Frequency> <num Threads> <parsedFile 1> ... <parsedFile n>");
			return;
		}
		String POS = args[0];//"N";
		String directory = args[1];//"/Users/akennedy/Research/buildMatrix/";
		String matrixName = args[2];//"finalMatrix_"+POS.toLowerCase();
		initialize(matrixName, POS, Integer.parseInt(args[3]), Integer.parseInt(args[4])); //N:35, V:10, A:35
		boolean dirCreated = createDirectory(directory, matrixName);
		if(!dirCreated){
			return;
		}
		...
		for(int i = 6; i < args.length; i++){
			String file = args[i];
			Thread t = new Thread(new BuildMatrixThreaded(file));
			t.start();
			threads.add(t);
	 ...
	 * @param colMin
	 */
	public static void initialize(String mn, String pos, int rowMin, int colMin){
		matrixName = mn;
		POS = pos;
	 ...
	 */
	public static void generateCRS() {
		try{
	 ...
	 * @throws IOException
	 */
	public static ArrayList<Integer> getLine(TreeMap<Integer, Integer> row, BufferedWriter bw) throws IOException{
	 ...
	 * @throws IOException
	 */
	private static void writeFeature(String word, BufferedWriter featureWriter, ArrayList<Integer> values) throws IOException {
		int nz = values.size();
		int tf = 0;
	 ...
	 */
	public static void generateCCS() {
		try {
	 ...
	 * @throws IOException
	 */
	private static void printBoundary(BufferedWriter boundaryWriter, String feature, int line) throws IOException {
	 ...
	 * constructor. 
	 */
	public static  void generateRowMap() {
		try{
	...
	private static int getCount(TreeMap<Integer, Integer> row) {
		int count = 0;
		for(int key : row.keySet()){
			if(columnOriginal2sorted[key] != -1){
				count++;
	 ...
	 */
	public static void generateColumnMap() {
		try{
	 ...
	 * @return
	 */
	public static boolean createDirectory(String directory, String matrixName) {
	 ...
	 * @param fname
	 */
	public BuildMatrixThreaded(String fname) {
		this.fname = fname;
	}
	 ...
	 * @param context
	 */
	private static synchronized void countPair(String wordStr, String contextStr) {
		int word = getWord(wordStr);
		int context = getContext(contextStr);
		...
		if(pairCounter.size() == word){
			...
			System.out.println("Missmatch: " + pairCounter.size() + " :: " + word);
		}
	}
	 ...
	 * @return
	 */
	private static synchronized int getWord(String word){
	 ...
	 * @return
	 */
	private static synchronized int getContext(String context){
